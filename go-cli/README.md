...
boru-driven command-line client **and** interactive REPL for the VisitorFrequenciesLucerneCh
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/visitor-frequencies-lucerne-ch-cli)
make build

# 2. See usage (words, entities, env vars)
./visitor-frequencies-lucerne-ch-cli --help
...
VISITOR_FREQUENCIES_LUCERNE_CH_BASE=https://api.example.com ./visitor-frequencies-lucerne-ch-cli list search

# 6. No arguments -> interactive REPL
./visitor-frequencies-lucerne-ch-cli
visitor-frequencies-lucerne-ch> list search
visitor-frequencies-lucerne-ch> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/visitor-frequencies-lucerne-ch-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/visitor-frequencies-lucerne-ch-cli list search
   ```
...
Run with no arguments to open a REPL (prompt `visitor-frequencies-lucerne-ch>`). Each line is
evaluated as its own boru expression:

```text
$ ./visitor-frequencies-lucerne-ch-cli
visitor-frequencies-lucerne-ch> list search
visitor-frequencies-lucerne-ch> /help
visitor-frequencies-lucerne-ch> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/visitor-frequencies-lucerne-ch-cli`. |
...
(`./visitor-frequencies-lucerne-ch-cli <expr>`) and interactively (the REPL), and expressions compose the
