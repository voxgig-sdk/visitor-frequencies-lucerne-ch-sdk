...
//    only - a typed VisitorFrequenciesLucerneChContext there would make every ctx
...
final class VisitorFrequenciesLucerneChOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new VisitorFrequenciesLucerneChContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new VisitorFrequenciesLucerneChSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new VisitorFrequenciesLucerneChResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new VisitorFrequenciesLucerneChError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new VisitorFrequenciesLucerneChResponse($clean['response']);
    ...
    public static function err_from_map($m): ?VisitorFrequenciesLucerneChError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new VisitorFrequenciesLucerneChError(is_string($code) ? $code : '', $msg);
    }
