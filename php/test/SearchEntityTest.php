...
class SearchEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = VisitorFrequenciesLucerneChSDK::test($seed, null);
        $seen = iterator_to_array($base->Search(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = VisitorFrequenciesLucerneChConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = VisitorFrequenciesLucerneChSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set VISITOR_FREQUENCIES_LUCERNE_CH_TEST_SEARCH_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function search_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("VISITOR_FREQUENCIES_LUCERNE_CH_TEST_SEARCH_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["VISITOR_FREQUENCIES_LUCERNE_CH_TEST_SEARCH_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["VISITOR_FREQUENCIES_LUCERNE_CH_TEST_LIVE"] === "TRUE") {
    ...
    $live = $env["VISITOR_FREQUENCIES_LUCERNE_CH_TEST_LIVE"] === "TRUE";
    return [
        "client" => $client,
        "data" => $entity_data,
        "idmap" => $idmap_resolved,
        "env" => $env,
        "explain" => $env["VISITOR_FREQUENCIES_LUCERNE_CH_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
