...
// VisitorFrequenciesLucerneCh SDK retry feature
...
class VisitorFrequenciesLucerneChRetryFeature extends VisitorFrequenciesLucerneChBaseFeature
{
    ...
    public function init(VisitorFrequenciesLucerneChContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (VisitorFrequenciesLucerneChContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(VisitorFrequenciesLucerneChContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(VisitorFrequenciesLucerneChContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
