...
class SearchEntity
{
    ...
    {
        if ($args) {
            $this->_data = VisitorFrequenciesLucerneChHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = VisitorFrequenciesLucerneChHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws VisitorFrequenciesLucerneChError on failure (item-5 convention).
     */
