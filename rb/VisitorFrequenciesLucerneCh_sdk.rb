...
require_relative 'VisitorFrequenciesLucerneCh_types'
...
class VisitorFrequenciesLucerneChSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = VisitorFrequenciesLucerneChHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = VisitorFrequenciesLucerneChHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, VisitorFrequenciesLucerneChFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    VisitorFrequenciesLucerneChUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = VisitorFrequenciesLucerneChHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = VisitorFrequenciesLucerneChHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = VisitorFrequenciesLucerneChHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => VisitorFrequenciesLucerneChError.new(
        "#{op}_allow",
        "VisitorFrequenciesLucerneChSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue VisitorFrequenciesLucerneChError => err
    ...
    if fetched.is_a?(Hash)
      status = VisitorFrequenciesLucerneChHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = VisitorFrequenciesLucerneChError.new(
        "graphql_error", "VisitorFrequenciesLucerneChSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = VisitorFrequenciesLucerneChSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
