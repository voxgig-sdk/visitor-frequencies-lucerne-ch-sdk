...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("VISITOR_FREQUENCIES_LUCERNE_CH_TEST_LIVE") == "TRUE" ||
		os.Getenv("VISITOR_FREQUENCIES_LUCERNE_CH_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("VISITOR_FREQUENCIES_LUCERNE_CH_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.VisitorFrequenciesLucerneChSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.VisitorFrequenciesLucerneChSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.VisitorFrequenciesLucerneChError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.VisitorFrequenciesLucerneChSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.VisitorFrequenciesLucerneChError{Msg: msg, Code: code}
}
