...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("VISITORFREQUENCIESLUCERNECH_TEST_LIVE") == "TRUE" ||
		os.Getenv("VISITORFREQUENCIESLUCERNECH_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("VISITORFREQUENCIESLUCERNECH_TEST_EXPLAIN"); explain != "" {
