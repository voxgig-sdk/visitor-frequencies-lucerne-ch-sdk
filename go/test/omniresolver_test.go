...
import (
	...
	omni "github.com/voxgig-sdk/visitor-frequencies-lucerne-ch-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.VisitorFrequenciesLucerneChError); is {
				out := map[string]any{
					"name":    "VisitorFrequenciesLucerneChError",
...
func omniCtx(arg any, client *sdk.VisitorFrequenciesLucerneChSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.VisitorFrequenciesLucerneChSDK); is {
			client = live
