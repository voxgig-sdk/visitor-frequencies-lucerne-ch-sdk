# VisitorFrequenciesLucerneCh SDK test runner
...
class VisitorFrequenciesLucerneChTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            VisitorFrequenciesLucerneChTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = VisitorFrequenciesLucerneChTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = VisitorFrequenciesLucerneChTestRunner.getenv("VISITOR_FREQUENCIES_LUCERNE_CH_TEST_LIVE")
        override = VisitorFrequenciesLucerneChTestRunner.getenv("VISITOR_FREQUENCIES_LUCERNE_CH_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = VisitorFrequenciesLucerneChTestRunner.getenv(key)
        ...
        explain = VisitorFrequenciesLucerneChTestRunner.getenv("VISITOR_FREQUENCIES_LUCERNE_CH_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if VisitorFrequenciesLucerneChTestRunner._test_control is not None:
            return VisitorFrequenciesLucerneChTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                VisitorFrequenciesLucerneChTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            VisitorFrequenciesLucerneChTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = VisitorFrequenciesLucerneChTestRunner.load_test_control()
...
def load_env_local():
    VisitorFrequenciesLucerneChTestRunner.load_env_local()


def env_override(m):
    return VisitorFrequenciesLucerneChTestRunner.env_override(m)


def entity_data(v):
    return VisitorFrequenciesLucerneChTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return VisitorFrequenciesLucerneChTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return VisitorFrequenciesLucerneChTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return VisitorFrequenciesLucerneChTestRunner.load_test_control()


def live_client_options():
    return VisitorFrequenciesLucerneChTestRunner.live_client_options()


def live_delay_ms():
    return VisitorFrequenciesLucerneChTestRunner.live_delay_ms()
