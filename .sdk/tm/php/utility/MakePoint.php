...
// VisitorFrequenciesLucerneCh SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class VisitorFrequenciesLucerneChMakePoint
{
    public static function call(VisitorFrequenciesLucerneChContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof VisitorFrequenciesLucerneChError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = VisitorFrequenciesLucerneChHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = VisitorFrequenciesLucerneChHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
