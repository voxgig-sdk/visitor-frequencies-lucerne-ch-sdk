...
module VisitorFrequenciesLucerneChUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = VisitorFrequenciesLucerneChResponse.new({})
    ...
    if fetch_err
      ...
      response = VisitorFrequenciesLucerneChResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = VisitorFrequenciesLucerneChResponse.new(fetched)
    else
