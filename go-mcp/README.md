...
[MCP](https://modelcontextprotocol.io) server exposing the VisitorFrequenciesLucerneCh SDK as
two agent tools — `visitor-frequencies-lucerne-ch_list` and `visitor-frequencies-lucerne-ch_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/visitor-frequencies-lucerne-ch-mcp)
make build

# 2. Provide credentials via the environment
export VISITOR_FREQUENCIES_LUCERNE_CH_APIKEY=sk_live_xxx
...
```jsonc
// visitor-frequencies-lucerne-ch_list: first page of records
{ "entity": "search" }
{ "entity": "search", "query": { } }

// visitor-frequencies-lucerne-ch_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/visitor-frequencies-lucerne-ch-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user visitor-frequencies-lucerne-ch \
     -- "$PWD"/dist/*/visitor-frequencies-lucerne-ch-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `visitor-frequencies-lucerne-ch_list` and `visitor-frequencies-lucerne-ch_load` tools now appear
   in new sessions. Ask the agent to *"list search using visitor-frequencies-lucerne-ch"*
   and it calls `visitor-frequencies-lucerne-ch_list` with `{"entity":"search"}`.
...
```sh
export VISITOR_FREQUENCIES_LUCERNE_CH_APIKEY=sk_live_xxx            # API key
export VISITOR_FREQUENCIES_LUCERNE_CH_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `visitor-frequencies-lucerne-ch_list` tool
...
### Call the `visitor-frequencies-lucerne-ch_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `visitor-frequencies-lucerne-ch_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `visitor-frequencies-lucerne-ch_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `VISITOR_FREQUENCIES_LUCERNE_CH_APIKEY` | API key sent with every request. |
| `VISITOR_FREQUENCIES_LUCERNE_CH_BASE` | Optional override of the API base URL. |
